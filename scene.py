...
슬롯 (구성 순서 = 이야기 순서):
    result    결과물    — 아이들이 만든 작품·화면. **첫 화면으로 가장 강하다**
    wide      전체샷    — 어떤 자리인지 보여준다. 애들이 밀집돼 있으면 더 좋다
    practice  실습샷    — 뭘 하고 있는지. 무슨 실습인지 보이면 더 좋다
    collab    협동샷    — 같이 머리 맞대는 장면. 없을 수도 있다
    tool      교구샷    — 교구로 뭔가 하는 장면. 교구가 있을 때만
...
# 아이들이 만든 결과물 화면이 첫 화면을 채웠다. 반대로 교실 뒷모습·현수막으로
# 시작한 것들은 하위였다. 릴스는 첫 1~2초에 넘길지 말지가 갈린다.
...
# 협동샷: 두세 명이 가까이 붙어 있다     (실측 4명 / 퍼짐 0.37)
COLLAB_MAX_FACES = 4
...
class Scene:
    kind: str = OTHER
    density: float = 0.0     # 밀집도 점수. 높을수록 애들이 붙어 있다
    faces: int = 0
    spread: float = 0.0
