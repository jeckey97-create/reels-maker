...
수업 사진은 보통 휴대폰 갤러리에 있습니다. **갤러리(구글 포토)와 구글 드라이브는
서로 다른 서비스이고 자동으로 동기화되지 않습니다.** 한 번은 옮겨주셔야 합니다.

*PC 에서 옮기기 — 가장 빠릅니다*

1. `photos.google.com` 접속
2. 왼쪽 `앨범` 에서 수업 앨범 열기
3. 첫 사진에 마우스를 올려 왼쪽 위 동그라미 클릭 → 마지막 사진 `Shift+클릭` (전체 선택)
4. 오른쪽 위 `⋮` → **다운로드** (zip 으로 받아집니다)
5. 압축을 풀어 `C:\reels\사진\경민it고` 같은 폴더에 넣기

*휴대폰에서 옮기기*

1. 구글 포토 앱에서 앨범 열기 → 사진 전체 선택
2. **공유** → **드라이브에 저장**
3. `내 드라이브 / 릴스 / 경민it고` 선택

드라이브에 넣으면 PC 의 `G:\내 드라이브\릴스\경민it고` 로 내려옵니다.
`watch_folder.py` 를 띄워두면 새 폴더가 생기는 대로 알아서 만듭니다.
...
프로그램(코드)은 **MIT 라이선스**로 자유롭게 쓰셔도 됩니다.
수업 홍보 같은 상업적 용도로 쓰셔도 되고, 고쳐서 쓰셔도 됩니다.

**다만 전자책은 별도 저작물이며 무단 복제·배포를 금합니다.**
