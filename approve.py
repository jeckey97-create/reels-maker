...
def cmd_show(state: dict, name: str) -> int:
    ...
    if not hit:
        ...
        return 2
    key, rec = hit
    print(f"[i] {Path(key).name}  ({rec.get('status')})")
    print(f"    영상: {rec.get('video')}")
...
def cmd_publish(state: dict, name: str) -> int:
    ...
    video_path = Path(rec.get("video", ""))
    if not video_path.exists():
        print(f"[!] 영상 파일이 없다: {video_path}")
        return 2

    if not cfg.PUBLIC_VIDEO_BASE:
        print("[!] .env 의 REELS_PUBLIC_BASE 가 비어 있다. Graph API 는 공개 URL 로만 영상을 받는다.")
        return 2
    if not graph_api.configured():
        print("[!] .env 의 IG_USER_ID / IG_ACCESS_TOKEN 이 비어 있다.")
        return 2

    caption = video_path.with_suffix(".txt").read_text(encoding="utf-8")
    video_url = f"{cfg.PUBLIC_VIDEO_BASE.rstrip('/')}/{video_path.name}"

    media_id = graph_api.publish_reel(video_url, caption, dry_run=False)
    rec["status"] = PUBLISHED
    rec["media_id"] = media_id
    save_state(state)
